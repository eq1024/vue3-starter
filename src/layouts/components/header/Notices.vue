...
<template>
  <n-popover placement="bottom" trigger="click" arrow-point-to-center class="!p-0">
    ...
    <n-tabs v-model:value="currentTab" type="segment" animated justify-content="space-evenly" class="w-430px box-border p-20px">
      <n-tab-pane :name="0">
        <template #tab>
          <n-space class="w-130px" justify="center" align="center">
            {{ $t('app.notifications') }}
            <n-badge class="" type="info" :value="groupMessage[0]?.filter(i => !i.isRead).length" :max="99" />
          </n-space>
        </template>
      ...
      <n-tab-pane :name="1">
        <template #tab>
          <n-space class="w-130px" justify="center" align="center">
            {{ $t('app.messages') }}
            <n-badge class="" type="warning" :value="groupMessage[1]?.filter(i => !i.isRead).length" :max="99" />
          </n-space>
        </template>
      ...
      <n-tab-pane :name="2">
        <template #tab>
          <n-space class="w-130px" justify="center" align="center">
            {{ $t('app.todos') }}
            <n-badge class="" type="error" :value="groupMessage[2]?.filter(i => !i.isRead).length" :max="99" />
          </n-space>
        </template>
